...
with st.sidebar:
    ...
    try:
        ...
        if rl_agent.is_available():
            ...
            st.divider()
            st.warning("RL Agent unavailable — install `stable-baselines3` and `gymnasium` for full analysis.")
    except ImportError:
        st.divider()
        st.warning("RL Agent unavailable — install `stable-baselines3` and `gymnasium` for full analysis.")
...
# =============================================================================
dashboard_tab, overview_tab, analysis_tab, news_tab, technical_tab, fundamentals_tab, backtest_tab = st.tabs(
    ["Dashboard", "Overview", "Analysis", "News & Sentiment", "Technical", "Fundamentals", "Backtest"]
)
