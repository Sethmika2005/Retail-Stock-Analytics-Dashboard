...
def render(selected, price_data, info, tech_score, tech_details, last_row,
    ...
    st.markdown('<div style="height:1px;background:#D0E8EA;margin:10px 0;"></div>', unsafe_allow_html=True)
    ...
    if sma50 and sma200:
        ...
        with val_col2:
            st.metric("50-Day Avg", f"${sma50:.2f}")
        with val_col3:
            st.metric("200-Day Avg", f"${sma200:.2f}")
        with val_col4:
            st.markdown("**Status**")
    ...
    with st.expander("Understanding Trend Analysis"):
        if sma50 and sma200:
            ...
            if sma50 > sma200:
                cross_desc = "**Golden Cross Active:** The 50-day average is above the 200-day average, indicating a bullish long-term trend."
            else:
                cross_desc = "**Death Cross Active:** The 50-day average is below the 200-day average, indicating a bearish long-term trend."

            if current_price > sma50 > sma200:
                trend_desc = "Price is above both averages in a strong uptrend configuration."
            elif current_price > sma50 and current_price > sma200:
                trend_desc = "Price is above both averages with bullish momentum."
            elif current_price > sma200:
                trend_desc = "Price is above the 200-day average but below the 50-day — short-term weakness in a longer-term uptrend."
            elif current_price < sma50 < sma200:
                trend_desc = "Price is below both averages in a strong downtrend configuration."
            else:
...
**Current Position:**
- Price vs 50-Day Avg: {price_vs_sma50:+.1f}%
- Price vs 200-Day Avg: {price_vs_sma200:+.1f}%

**Key Signals:**
- **Golden Cross:** 50-day average crosses above 200-day average — strong buy signal
- **Death Cross:** 50-day average crosses below 200-day average — strong sell signal
            """)
...
- **Below 30:** Oversold - potential bounce
    ...
    macd_sig_preview = price_data["MACD_SIGNAL"].iloc[-1] if "MACD_SIGNAL" in price_data.columns else 0
    macd_direction = "bullish" if macd_val > macd_sig_preview else "bearish"
    macd_hist_preview = price_data["MACD_HIST"].iloc[-1] if "MACD_HIST" in price_data.columns else 0
    macd_strength = "strengthening" if abs(macd_hist_preview) > abs(price_data["MACD_HIST"].iloc[-2] if "MACD_HIST" in price_data.columns and len(price_data) > 1 else 0) else "weakening"
    st.caption(f"MACD shows {macd_direction} momentum that is {macd_strength}.")
    ...
    hist_strength = "strengthening" if abs(macd_hist) > abs(macd_val) * 0.3 else "fading"
    macd_col1, macd_col2 = st.columns(2)
    with macd_col1:
        st.markdown("**Momentum**")
        st.markdown(f"<span style='color:{macd_color}; font-weight:600; font-size:18px;'>{macd_signal}</span> <span style='color:#5A7D82; font-size:13px;'>({hist_strength})</span>", unsafe_allow_html=True)
    with macd_col2:
        st.metric("MACD", f"{macd_val:.3f}", delta=f"Histogram: {macd_hist:+.3f}")
...
- **Bearish Crossover:** MACD crosses below Signal
    ...
    with vol_col1:
        ...
        st.metric("Volume Score", f"{volume_score}/100 ({vol_label})")
    with vol_col2:
        ...
        rel_vol_label = "Above average" if rel_vol > 1.0 else "Below average"
        st.metric("Relative Volume", f"{rel_vol:.2f}x ({rel_vol_label})")
    with vol_col3:
...
**Volume SMAs:**
- **20-Day Avg:** Short-term average volume (blue line)
- **50-Day Avg:** Medium-term average volume (red line)
- Volume above the 20-day average = above-average participation
        """)
