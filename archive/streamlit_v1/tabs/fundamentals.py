...
def render(selected, info, financials, all_stocks_df, filtered_df,
    ...
    if prof_data is not None and len(prof_dates) > 0:
        ...
        if ni_col:
            ...
            # Dynamic one-liner interpretation
            roe_val_snap = info.get("returnOnEquity")
            if ni_values and len(ni_values) >= 2:
                ni_trend = "grown" if ni_values[-1] > ni_values[0] else "declined"
                prof_caption = f"Net income has {ni_trend} over the period shown."
                if roe_val_snap:
                    roe_assess = "strong" if roe_val_snap > 0.15 else "moderate" if roe_val_snap > 0.10 else "weak"
                    prof_caption += f" ROE of {roe_val_snap*100:.1f}% is {roe_assess}."
                st.caption(prof_caption)
    else:
    ...
    if growth_data is not None and len(growth_dates) > 0:
        ...
        if rev_col:
            ...
            # Dynamic one-liner interpretation
            if rev_values and len(rev_values) >= 2:
                rev_trend = "grown" if rev_values[-1] > rev_values[0] else "declined"
                rev_caption = f"Revenue has {rev_trend} over the period shown."
                rev_growth_snap = info.get("revenueGrowth")
                if rev_growth_snap:
                    rev_assess = "strong" if rev_growth_snap > 0.15 else "moderate" if rev_growth_snap > 0.05 else "slow"
                    rev_caption += f" Current growth of {rev_growth_snap*100:.1f}% is {rev_assess}."
                st.caption(rev_caption)
    ...
    if lev_data is not None and len(lev_dates) > 0:
        ...
        if debt_col or equity_col:
            ...
            # Dynamic one-liner interpretation
            de_snap = info.get("debtToEquity")
            if de_snap is not None:
                de_assess = "conservative" if de_snap < 50 else "moderate" if de_snap < 100 else "high" if de_snap < 200 else "very high"
                st.caption(f"Debt-to-Equity ratio of {de_snap:.0f} indicates {de_assess} leverage for this company.")
    else:
    ...
    if show_peer_comparison:
        ...
        if comparison_data:
            ...
            styled_df = df.style.map(highlight_assessment, subset=['Assessment'])
