...
STRATEGY_COLORS = {
    "Novel Hybrid (EMA+ATV+RSI+RL)": "#4CAF50",
}


def render(selected, price_data, info, market_regime):
    """Render the Backtest comparison tab."""
    st.subheader("Strategy Backtest")
    st.caption("Backtest the Novel Hybrid strategy (EMA+ATV+RSI+RL with regime-aware logic) on historical data.")
    ...
    if st.button("Run Backtest", type="primary"):
        with st.spinner(f"Running {bt_period} backtest for {selected}..."):
            _run_and_display_backtest(
                selected, price_data, info, market_regime,
                bt_days, bt_period, simulate_strategy, calculate_backtest_metrics,
                get_strategy_functions, bt_compute_indicators,
            )
    else:
        st.info(f"Click 'Run Backtest' to test the Novel Hybrid strategy over {bt_period}.")
...
def _run_and_display_backtest(
    ...
    for name in strategy_names:
        ...
        table_rows.append({
            ...
            "Risk-Adj Return": m["sharpe_ratio"],
            "Downside Risk": m["sortino_ratio"],
    ...
    numeric_cols = ["Total Return %", "Annual Return %", "Risk-Adj Return", "Downside Risk",
    ...
    # Dynamic one-liner: $10K investment outcome
    for name in strategy_names:
        curve = all_results[name]["equity_curve"]
        if curve and len(curve) >= 2:
            ...
            if start_val and start_val > 0:
                growth_factor = end_val / start_val
                hypothetical = 10000 * growth_factor
                st.caption(f"If you had invested $10,000 using **{name}**, it would now be worth **${hypothetical:,.0f}**.")
    ...
    if signals:
        ...
        with t_col4:
            st.metric("Risk-Adj Return", f"{m['sharpe_ratio']:.2f}")
