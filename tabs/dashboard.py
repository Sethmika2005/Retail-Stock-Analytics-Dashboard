...
def render(selected, price_data, info, last_row, change_pct,
    ...
    signal_explanations = {
        "BUY": "This signal is generated by combining multiple technical indicators and AI analysis. The majority of indicators are currently pointing toward upside potential.",
        "SELL": "This signal is generated by combining multiple technical indicators and AI analysis. The majority of indicators are currently pointing toward downside risk.",
        "HOLD": "This signal is generated by combining multiple technical indicators and AI analysis. Indicators are currently mixed, with no clear consensus on direction.",
    }
    ...
    if confidence >= 70:
        conf_explain = "Confidence measures how many indicators agree on the current signal. At this level, most indicators are aligned, suggesting a more reliable signal."
    elif confidence >= 50:
        conf_explain = "Confidence measures how many indicators agree on the current signal. At this level, some indicators conflict with each other, meaning the signal carries more uncertainty."
    else:
        conf_explain = "Confidence measures how many indicators agree on the current signal. At this level, indicators are largely disagreeing, meaning the signal should be interpreted with caution."
    ...
    if paper1_details:
        ...
        rsi_gate_map = {"passed": ("Passed", "#10B981"),
                        "blocked_overbought": ("Blocked", "#EF4444"),
                        "blocked_oversold": ("Blocked", "#EF4444"),
                        "n/a": ("N/A", "#94A3B8")}
        ...
        if rl_sig:
            ...
            ai_row = _signal_row("AI Model", f"{rl_sig} \u2014 {ai_status}", ai_clr)
        else:
            ai_row = _signal_row("AI Model", "Not Active", "#94A3B8")
    else:
    ...
    with sig_cols[3]:
        ...
        <div style="{BOX}">
            ...
            <div style="line-height:1.6;font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif;">
                {_signal_row("Trend", cross_name, cross_clr)}
                {_signal_row("Volume", atv, atv_clr)}
    ...
    with hc_col:
        ...
        bars_html = _progress_bar(f"Tech Score \u2014 {tech_label_text}", tech_score, "#0097A7")
        bars_html += _progress_bar(f"Volume \u2014 {vol_label_text}", volume_score, "#0097A7")
        bars_html += _progress_bar(f"Momentum (RSI) \u2014 {rsi_zone}", rsi_safe, rsi_bar_color)
